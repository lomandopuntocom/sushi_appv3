    ...
    <main className="admin-blog">
      ...
      <section className="admin-blog__layout">
        <form className="admin-blog__form" onSubmit={handleSubmit}>
          ...
          <label>
            Contenido Markdown
            <textarea name="contenido" value={form.contenido} onChange={handleChange} rows="10" required />
          </label>
          <div className="admin-blog__actions">
            <button type="submit">{editingId ? 'Guardar cambios' : 'Publicar post'}</button>
              ...
              {posts.map((post) => (
                <article key={post.id} className="admin-blog__item">
                  <div>
                    ...
                    <span>{post.autor || post.usuario?.nombre || 'Sushi App'} - {new Date(post.fecha).toLocaleDateString()}</span>
                  </div>
                  <div className="admin-blog__actions">
                    <button type="button" onClick={() => handleEdit(post)}>Editar</button>
                    <button type="button" onClick={() => handleDelete(post.id)}>Eliminar</button>
                  </div>
                </article>
              ))}
            </div>
