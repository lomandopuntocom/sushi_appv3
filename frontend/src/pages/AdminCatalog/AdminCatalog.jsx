    ...
    <main className="admin-catalog">
      ...
      <section className="admin-catalog__layout">
        <form className="admin-catalog__form" onSubmit={handleSubmit}>
          ...
          <label>
            Imagen
            <input name="image" value={form.image} onChange={handleChange} placeholder="/img/sushi.png" />
          </label>
          ...
          <div className="admin-catalog__actions">
            <button type="submit">{editingId ? 'Guardar cambios' : 'Crear platillo'}</button>
            {editingId && <button type="button" onClick={resetForm}>Cancelar</button>}
          </div>
        </form>

        <form className="admin-catalog__category" onSubmit={handleCreateCategory}>
            ...
            {platillos.map((platillo) => (
              <article key={platillo.id} className="admin-catalog__row">
                <div>
                  ...
                  <span>{platillo.categoria?.nombre} - ${parseFloat(platillo.precio).toFixed(2)}</span>
                </div>
                <div className="admin-catalog__row-actions">
                  <button type="button" onClick={() => handleEdit(platillo)}>Editar</button>
                  <button type="button" onClick={() => handleDelete(platillo.id)}>Eliminar</button>
                </div>
              </article>
            ))}
          </div>
